...
from genre.wiki_title_trie import get_prefix_allowed_tokens_fn
...
def evaluate_kilt_dataset(
    ...
    for docs in batch_it(iter_, batch_size):
        ...
        for doc, out in zip(docs, outputs):
            ...
            out_set = []
            for prov in out:
                out_set.extend(prov['text'].split(' |'))
            doc["output"] = [
                {
                    "answer": "",
                    "provenance": [
                        {
                            "wikipedia_id": title2id.get(prov, None),
                            "title": prov,
                            "score": 0,
                        }
                        for prov in list(dict.fromkeys(out_set))
                    ],
                }
            ]
